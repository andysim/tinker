...
c
     ...
     &                - 2.0d0*qiyz*dufld(3,i) - qixz*dufld(2,i)
     &                + 2.0d0*qiyz*dufld(6,i) + qixy*dufld(4,i)
     &                + (qiyy-qizz)*dufld(5,i)
         trqi(2) = diz*ufld(1,i) - dix*ufld(3,i)
     &                + 2.0d0*qixz*dufld(1,i) + qiyz*dufld(2,i)
     &                - 2.0d0*qixz*dufld(6,i) - qixy*dufld(5,i)
     &                + (qizz-qixx)*dufld(4,i)
         trqi(3) = dix*ufld(2,i) - diy*ufld(1,i)
     &                - 2.0d0*qixy*dufld(1,i) - qiyz*dufld(4,i)
     &                + 2.0d0*qixy*dufld(3,i) + qixz*dufld(5,i)
     &                + (qixx-qiyy)*dufld(2,i)
         trqi(1) = f * trqi(1)
         trqi(2) = f * trqi(2)
         trqi(3) = f * trqi(3)
         call torque (i,trqi,frcxi,frcyi,frczi,dep)
      end do
c
