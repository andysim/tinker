...
c
      ...
      allocate (strc2(nfft1,nfft2,nfft3))
      ...
      if(use_polar) then
         if (poltyp .eq. "MUTUAL") then
            ...
            call fftfront
c
c           compute the µ-µ contribution to the structure factor
c           and consolidate the uind and uinp grids
c
            do k = 1, nfft3
               do j = 1, nfft2
                  do i = 1, nfft1
                     strc2(i,j,k) = strc2(i,j,k) +
     &                 ( qgrid(1,i,j,k)*qgrip(1,i,j,k)
     &                 + qgrid(2,i,j,k)*qgrip(2,i,j,k) )
                      qgrid(1,i,j,k) = qgrid(1,i,j,k) - qgrip(2,i,j,k)
                      qgrid(2,i,j,k) = qgrid(2,i,j,k) + qgrip(1,i,j,k)
                  end do
               end do
            end do
         else  ! Mutual / OPT / Direct
c
c           this is the OPT/Direct algorithm
c
            do i = 1, 3
...
c
            ...
            call fftfront
c
c           add in PT structure factor contributions the virial
c
            if(poltyp(1:3) .eq. 'OPT') then
               do l = 0, ptmaxord-1
                  do m = 0, ptmaxord-1
     ...
     &                              )
         ...
         endif ! mutual / OPT / direct
c
c        compute the structure factor contribution to the virial
c
         do k = 1, nfft3
            do j = 1, nfft2
               do i = 1, nfft1
                  strc2(i,j,k) = strc2(i,j,k)
c                 perm-perm
     &              + permgridf(1,i,j,k)**2 + permgridf(2,i,j,k)**2
c                 perm-µ and µ-perm
     &              + permgridf(1,i,j,k)*qgrid(1,i,j,k)
     &              + permgridf(1,i,j,k)*qgrid(2,i,j,k)
     &              - permgridf(2,i,j,k)*qgrid(1,i,j,k)
     &              + permgridf(2,i,j,k)*qgrid(2,i,j,k)
               end do
            end do
         end do
c
c        form fractional permanent multipoles
c
         do i = 1, npole
            cmp(1,i) = rpole(1,i)
            cmp(2,i) = rpole(2,i)
         ...
         call cmp_to_fmp (cmp,fmp)
c
c        form fractional induced multipoles
c
         do i = 1, 3
            a(1,i) = dble(nfft1) * recip(i,1)
            a(2,i) = dble(nfft2) * recip(i,2)
...
c
c        the permanent moment potential derivatives were cached earlier
c        when we computed the permanent field for the dipole calculation
c
         do i = 1, npole
            do j = 1, 20
               fphiperm(j,i) = electric * fphiperm(j,i)
            end do
         end do
         call fphi_to_cphi (fphiperm,cphi)
c
...
c
         ...
         do i = 1, npole
            ...
            f3 = 0.0d0
            do k = 1, 10
               e = e + fmp(k,i)*fphiperm(k,i)
               f1 = f1 + fmp(k,i)*fphiperm(deriv1(k),i)
               f2 = f2 + fmp(k,i)*fphiperm(deriv2(k),i)
               f3 = f3 + fmp(k,i)*fphiperm(deriv3(k),i)
            end do
            f1 = dble(nfft1) * f1
            f2 = dble(nfft2) * f2
            f3 = dble(nfft3) * f3
...
c
c        perform the scalar sum to find the structure factor contribution
c        to the virial, and compute qfac, needed below
c
         call ssum(strc2, vxx, vyx, vzx, vyy, vzy, vzz)
c
...
c
         ...
         do i = 1, npole
            ...
            do k = 1, 3
               ...
               j2 = deriv2(k+1)
               j3 = deriv3(k+1)
               e = e + fuind(k,i)*fphiperm(k+1,i)
               f1 = f1 + (fuind(k,i)+fuinp(k,i))*fphiperm(j1,i)
               f2 = f2 + (fuind(k,i)+fuinp(k,i))*fphiperm(j2,i)
               f3 = f3 + (fuind(k,i)+fuinp(k,i))*fphiperm(j3,i)
c              add the µ-µ terms in, if needed
               if (poltyp.eq.'MUTUAL') then
...
c
         do i = 1, npole
            do j = 2, 4
               ...
               cphip(j) = 0.0d0
               do k = 2, 4
                  cphim(j) = cphim(j) + ftc(j,k)*fphiperm(k,i)
...
c
         if (poltyp(1:3) .eq. 'OPT') then
            do l = 0, ptmaxord-1
               ...
               do m = 0, ptmaxord-1
                  ...
                  do i = 1, npole
                     do j = 1, 3
                        fuind(j,i) = a(j,1)*ptuind(1,i,m)
     &                             + a(j,2)*ptuind(2,i,m)
     &                             + a(j,3)*ptuind(3,i,m)
                        fuinp(j,i) = a(j,1)*ptuinp(1,i,m)
     &                             + a(j,2)*ptuinp(2,i,m)
     &                             + a(j,3)*ptuinp(3,i,m)
                     end do
                  end do
     ...
     &                          + fuinp(k,i)*fphid(j3,i)
                     ...
                     f3 = 0.5d0 * dble(nfft3) * f3
                     frc(1,i) = 
     &                   recip(1,1)*f1+recip(1,2)*f2+recip(1,3)*f3
                     frc(2,i) =
     ...
     &                                    +fphip(4,i)*fuind(3,i))
                  ...
                  do i = 1, npole
                     ii = ipole(i)
                     dep(1,ii) = dep(1,ii) + c*frc(1,i)
                     dep(2,ii) = dep(2,ii) + c*frc(2,i)
                     dep(3,ii) = dep(3,ii) + c*frc(3,i)
                  end do
               enddo
            enddo
         endif
...
c
         do k = 1, nfft3
            do j = 1, nfft2
               do i = 1, nfft1
                  strc2(i,j,k) = strc2(i,j,k)
     &              + qgrid(1,i,j,k)**2 + qgrid(2,i,j,k)**2
               end do
            end do
         end do
c
