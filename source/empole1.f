...
c OPT IMPLEMENTATION
               if (poltyp.eq.'DIRECT' .or. poltyp(1:3).eq.'OPT') then
c OPT IMPLEMENTATION
...
c
     ...
     &                       + fuinp(k,i)*fphid(j3,i)
               ...
               do i = 1, npole
                  ii = ipole(i)
                  dep(1,ii) = dep(1,ii) + ptcoefs(l+m+1)*frc(1,i)
                  dep(2,ii) = dep(2,ii) + ptcoefs(l+m+1)*frc(2,i)
                  dep(3,ii) = dep(3,ii) + ptcoefs(l+m+1)*frc(3,i)
               end do
            enddo
         enddo
c OPT IMPLEMENTATION
            ...
            do m = 0, ptmaxord-1
               if(abs(ptcoefs(l+m+1)) .lt. 1D-6) cycle
                do k = 1, nfft3
                   do j = 1, nfft2
                      do i = 1, nfft1
                         strc2(i,j,k) = strc2(i,j,k) + ptcoefs(l+m+1)*
     &                     ( uindgridf(1,i,j,k,l)*uinpgridf(1,i,j,k,m)
     &                     + uindgridf(2,i,j,k,l)*uinpgridf(2,i,j,k,m) )
                      end do
                   end do
                end do
            enddo
         enddo
         call ssum(strc2, 1.0d0, vxx, vyx, vzx, vyy, vzy, vzz)
      endif
...
c
     ...
     &                  + gridd(2,k1,k2,k3)*gridp(2,k1,k2,k3)
      ...
      subroutine ssum(str2,scalefac,vxx,vyx,vzx,vyy,vzy,vzz)
      use pme
      use chgpot
      use ewald
      use boxes
